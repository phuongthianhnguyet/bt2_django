...
triển khai hoàn toàn bằng **Docker Compose** gồm 3 container độc lập.
...
|---|---|---|
| `camdo_mariadb` | Cơ sở dữ liệu chính (MariaDB 10.11) | 3306 |
| `camdo_django` | Ứng dụng web Django | 8000 |
| `camdo_phpmyadmin` | Giao diện kiểm tra CSDL trực quan | 8088 |

Toàn bộ hệ thống chạy trên **Ubuntu VM**, được public ra Internet thông qua
**Cloudflare Tunnel** — không cần mở port hay cấu hình router.

---

## Tính năng

- Quản lý đầy đủ 4 bảng nghiệp vụ: **Khách hàng**, **Hợp đồng**,
  **Tài sản cầm đồ**, **Lịch sử thanh toán**
- Trang `/admin` do Django tự sinh: hỗ trợ thêm / sửa / xoá trực tiếp,
  các trường khoá ngoại hiển thị dạng dropdown thay vì phải nhớ ID
- Trang chủ `/` liệt kê danh sách hợp đồng **đến hạn chưa chuộc**,
  render bằng template Django (cú pháp Jinja2)
- Kiểm chứng khoá ngoại (FK lưu ID số nguyên) qua phpMyAdmin tại `:8088`
